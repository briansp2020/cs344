                   ...
                   const float* const filter, const int filterWidth)
{
  ...
  int idy = hipBlockIdx_y * hipBlockDim_y + hipThreadIdx_y;
  int offset_1d = idy * numCols + idx;
  ...
  if (idx < numCols && idy < numRows)
  {
    ...
    for (int r = 0; r < filterWidth; r++)
    {
      ...
      {
        float filter_value = filter[r * filterWidth + c];
        int image_x = min(max(0, idx - filterWidth/2 + c), numCols - 1);
        int image_y = min(max(0, idy - filterWidth/2 + r), numRows - 1);
        unsigned char img_value = inputChannel[image_y * numCols + image_x];
        
        accum += img_value * filter_value;
      }
    }
    outputChannel[offset_1d] = (unsigned char)(accum);
  }
}
                      ...
                      unsigned char* const blueChannel)
{
  ...
  if (idx < numCols && idy < numRows)
  {
    uchar4 rgba = inputImageRGBA[offset_1d];
    redChannel[offset_1d] = rgba.x;
    greenChannel[offset_1d] = rgba.y;
    blueChannel[offset_1d] = rgba.z;
  }
}
...
void recombineChannels(const unsigned char* const redChannel,
                       ...
                       int numCols)
{
  const int2 thread_2D_pos = make_int2( hipBlockIdx_x * hipBlockDim_x + hipThreadIdx_x,
                        ...
                        const int filterWidth)
{
  ...
  const dim3 blockSize (NUM_THREAD_X, NUM_THREAD_Y);

  //TODO:
  //Compute correct grid size (i.e., number of blocks per kernel launch)
  //from the image size and and block size.
  const dim3 gridSize ((numCols - 1)/NUM_THREAD_X + 1, (numRows - 1)/NUM_THREAD_Y + 1);
